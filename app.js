...
/* Obtendo as informações, que estão na index, usando a DOM.
- aqui estamos obtendo os valores que precisamos de cada elemento através do seu ID e armazenando-os em uma constante */
...
// usando o Método de Date - getFullYear - para pegar o ano atual e somando 1 a ele para determinar o próximo ano.
const nextYear = new Date().getFullYear() + 1 

// Template Literal ou Template String permite uma interpolção simplificada. (se caracteriza pela String sempre entre crases (backticks ``))
// Exemplo do execício: `January 01 ${nextYear} 00:00:00`

/* A interpolação de strings é uma característica que permite injectar variáveis, chamadas de funções, e expressões aritméticas (através de espaço reservado (usando dollar sign + brackets ${})) directamente numa string sem utilizar caracteres de concatenação ou de fuga para strings de várias linhas ou ainda de escape \ no uso de apóstrofo como em haven\´t. */

/* Dica Atalho: quando seleciono um item, como a aspas de abertura da String,
e pressiono Ctrl + D, automaticamente eu seleciono as aspas do final e posso mudar as duas ao mesmo tempo */

const newYearTime = new Date(`January 01 ${nextYear} 00:00:00`) // indicando o momento de virada do ano, inserindo o ano dinamicamente
...
// Date é um Objeto Construtor para trabalharmos com data no JS, esse Objeto possuí vários Métodos, como o getFullYear usado acima na obtenção do ano atual.

// console.log(newYearTime)

// Dica Inspecionar: é possível vermos os métodos do construtor, clicando em Sources, inserindo um breakpoint na declaração da const, clicando em F5 para atualizar a página e em F10 para o passo seguinte no debbug. Depois ao pausarmos o mouse sobre a const no console.log, veremos que a função é um Object Prototype, e conseguiremos ver os Métodos e Propriedades que ele herda.

// O JavaScript é frequentemente descrito como uma linguagem baseada em protótipos — para fornecer herança, os objetos podem ter um objeto de protótipo, que atua como um objeto de modelo do qual herda métodos e propriedades. https://developer.mozilla.org/pt-BR/docs/Learn/JavaScript/Objects/Object_prototypes

// ANDAMENTO, até este ponto criamos uma const newYearTime, recebendo a função Date('com um valor definido: mes dia ano hh:mm:ss'), depois outra const nextYear a função Date com o Método getFullYear, para podermos inserir o ano dinamicamente (+ 1, pois queremos o ano que virá) dentro de newYearTime.
...
// Atribuo o próximo ano obtido na constante nextYear, através do Método de Date, à variável, e assim devolvo para o elemento da DOM, associado à Constante, o valor obtido.
nextYearContainer.textContent = nextYear

/* #### REFATORADA abaixo A FUNÇÃO DO updateCountdown ####
...
// ... e em seguida chamei a função de processamento do DISPLAY DO COUNTDOWN como parâmentro da função setTimeout, e passei um segundo parâmetro de tempo em milissegundos para indicar quando deve ser executada após o carregamento.
